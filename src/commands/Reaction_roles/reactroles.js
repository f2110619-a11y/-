...
async function handleDeletePanel(btnInteraction, rootInteraction, panelData, panels, guildId, guild, client, collector, buttonCollector) {
    ...
    // ═══════════════════════════════════════════════════════════════════════════════
// НОВЫЕ АДМИН КОМАНДЫ ДЛЯ ВЫДАЧИ/СНЯТИЯ РОЛЕЙ (ДОБАВИТЬ В КОНЕЦ ФАЙЛА)
// ═══════════════════════════════════════════════════════════════════════════════
...
async function handleAddRole(interaction) {
    ...
    if (!botMember.permissions.has(PermissionFlagsBits.ManageRoles)) {
        throw createError(
            ...
            ErrorTypes.PERMISSION,
            '❌ У меня нет права "Управление ролями".'
        );
    }
    ...
    if (!executorMember.permissions.has(PermissionFlagsBits.Administrator)) {
        if (role.position >= executorMember.roles.highest.position) {
            throw createError(
                'Роль выше вашей',
                ErrorTypes.PERMISSION,
                `❌ Роль "${role.name}" находится выше вашей высшей роли в иерархии.`
            );
        }
    }
    ...
    if (role.position >= botMember.roles.highest.position) {
        throw createError(
            'Роль выше роли бота',
            ErrorTypes.PERMISSION,
            `❌ Роль "${role.name}" находится выше моей высшей роли. Переместите мою роль выше в иерархии.`
        );
    }
    ...
    if (role.managed) {
        throw createError(
            'Управляемая роль',
            ErrorTypes.VALIDATION,
            `❌ Роль "${role.name}" является управляемой (роль бота или интеграции).`
        );
    }
    
    if (role.id === interaction.guild.id) {
        throw createError('Роль @everyone', ErrorTypes.VALIDATION, '❌ Нельзя выдать роль @everyone.');
    }
    ...
    await logRoleToChannel(interaction, {
        action: 'add',
        targetUser,
        role,
    ...
    logger.info(`[ROLE ADD] ${interaction.user.tag} выдал роль ${role.name} пользователю ${targetUser.tag}`);
}
...
async function handleRemoveRole(interaction) {
    ...
    await InteractionHelper.safeEditReply(interaction, { embeds: [embed] });
    logger.info(`[ROLE REMOVE] ${interaction.user.tag} снял роль ${role.name} с пользователя ${targetUser.tag}`);
}
...
async function handleListRoles(interaction) {
    ...
    if (roles.size === 0) {
        const embed = errorEmbed('📋 Нет ролей', `У пользователя ${targetUser.toString()} нет никаких ролей.`);
        return await interaction.reply({ embeds: [embed], ephemeral: true });
    }
    
    const rolesList = roles.map(role => `${role.toString()}`).join('\n');
    ...
    const embed = {
        ...
        fields: [
            { name: '📊 Всего ролей', value: `${roles.size}`, inline: true },
            { name: '🎭 Высшая роль', value: roles.first()?.toString() || 'Нет', inline: true }
        ],
        timestamp: new Date()
    };
...
async function logRoleToChannel(interaction, { action, targetUser, role, executor, reason }) {
    ...
    try {
        ...
        const logEmbed = {
            ...
            thumbnail: { url: targetUser.displayAvatarURL() },
            fields: [
                { name: '👤 Пользователь', value: `${targetUser.toString()} \`${targetUser.tag}\``, inline: false },
                { name: '🎭 Роль', value: `${role.toString()} \`${role.name}\``, inline: true },
                { name: '⚙️ Выдал/снял', value: `${executor.toString()} \`${executor.tag}\``, inline: true },
        ...
        logger.error('Ошибка при отправке лога:', error);
    }
}
}
